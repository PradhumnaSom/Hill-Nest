...
const quickActions = [
  {
    icon: "🛏️",
    label: "Browse Rooms",
    desc: "Explore all available rooms",
    href: "/rooms",
...
function StatCard({
  ...
  return (
    <div
      className={`dash-card rounded-2xl border ${colorClass} p-5 animate-fade-up`}
      style={{ animationDelay: delay }}
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-2xl">{icon}</span>
        <span className="text-3xl font-extrabold text-gray-900 font-serif animate-count-up">
          {value}
        </span>
      </div>
      ...
      <div className="bg-gradient-to-r from-green-900 via-green-800 to-emerald-700 text-white">
        <div className="max-w-5xl mx-auto px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/rooms"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/15 hover:bg-white/25 text-white text-sm font-semibold transition-all duration-200 border border-white/20"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 hover:bg-red-500/30 border border-white/20 hover:border-red-400/50 text-white text-sm font-semibold transition-all duration-200"
            >
              <span>→</span> Logout
            </button>
          </div>
        ...
        {status === "error" && (
          <div className="bg-red-50 border border-red-200 rounded-3xl p-8 text-center animate-fade-up">
            ...
            <button
              ...
              className="mt-4 px-5 py-2 rounded-xl bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="space-y-7">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden animate-fade-up">
              ...
              <div className="p-7 flex flex-col sm:flex-row items-start sm:items-center gap-6">
                ...
                <div className="relative flex-shrink-0">
                  <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-green-700 to-emerald-500 flex items-center justify-center text-3xl font-bold text-white shadow-lg shadow-green-900/20 font-serif">
                    {initials}
                  </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="text-xs font-semibold uppercase tracking-widest text-green-700 bg-green-50 border border-green-200 px-2.5 py-1 rounded-full">
                      {user.role}
                    </span>
                    <span className="text-xs text-gray-400">· Active member</span>
                  </div>
                  <h1 className="text-2xl font-bold text-gray-900 font-serif truncate">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {quickActions.map(({ icon, label, desc, href, color, border, iconBg, textColor }) => (
                  <Link
                    key={label}
                    href={href}
                    className={`dash-card group flex items-start gap-4 p-5 rounded-2xl bg-gradient-to-br ${color} border ${border} hover:shadow-md transition-all duration-300`}
                  >
                    <div className={`w-11 h-11 ${iconBg} rounded-xl flex items-center justify-center text-xl flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                      {icon}
                    </div>
                    <div>
                    ...
                    <svg
                      className={`w-4 h-4 ml-auto mt-0.5 ${textColor} opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-300`}
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-7 animate-fade-up delay-400">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link
                  href="/bookings"
                  className="text-sm font-semibold text-green-700 hover:text-green-800 hover:underline underline-offset-2 transition-colors"
                >
                  View all →
                </Link>
              </div>
              ...
              {recentBookings.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <Link
                    href="/rooms"
                    className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-green-700 text-white text-sm font-semibold hover:bg-green-800 transition-colors"
                  >
                    Explore Rooms
                  </Link>
                </div>
                ...
                <div className="divide-y divide-gray-50">
                  {recentBookings.map((booking, i) => (
                    <div
                      key={booking._id}
                      className="py-4 flex items-center justify-between gap-4 animate-fade-up"
